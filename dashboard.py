...
<style>
:root{
  --cafe-oscuro: #342311;   /* texto RAICES, color principal de marca */
  --rojo-vivo: #c81e1e;     /* anillo / subrayado, version atenuada para UI (el #fe0000 puro del logo es muy fuerte para fondos de UI) */
  --rojo-coral: #dc4750;    /* subrayado secundario */
  --naranja: #fa5302;       /* atardecer */
  --dorado: #ffa808;        /* atardecer claro */
  --crema: #FDF8F0;         /* fondo general, tono crema calido a juego con el atardecer */
}
body{font-family:Arial,sans-serif;background:var(--crema)}
.card{background:#fff;border-radius:10px;box-shadow:0 1px 4px rgba(52,35,17,.12)}
...
.b-EN_PREPARACION{background:#FCE4D6;color:#9A3B12}
.b-ENTREGADO{background:#D6D8D9;color:#383D41}
.b-CANCELADO{background:#F8D7DA;color:#842029}
select{border:1px solid var(--dorado);border-radius:6px;padding:4px 8px;font-size:12px;background:#fff;color:var(--cafe-oscuro)}
.tab{padding:8px 16px;border-radius:8px 8px 0 0;cursor:pointer;font-size:13px;font-weight:600;color:#9b8b7a}
.tab.active{background:#fff;color:var(--cafe-oscuro);box-shadow:0 -1px 3px rgba(52,35,17,.1);border-bottom:2px solid var(--naranja)}
.logo-img{height:56px;border-radius:50%;box-shadow:0 0 0 2px var(--rojo-vivo),0 0 0 4px #fff,0 0 0 5px var(--rojo-coral)}
.titulo-marca{color:var(--cafe-oscuro)}
.subrayado-marca{height:2px;background:linear-gradient(90deg,var(--rojo-vivo),var(--rojo-coral));width:100%;margin-top:2px}
.btn-refrescar{background:var(--cafe-oscuro);color:var(--dorado)}
